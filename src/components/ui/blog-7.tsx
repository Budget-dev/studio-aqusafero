    ...
    <section className="py-12 md:py-24 bg-white">
      <div className="container mx-auto flex flex-col items-center gap-8 md:gap-16 px-4">
        <div className="text-center">
          ...
          <h2 className="mb-2 text-pretty text-2xl font-black font-headline text-slate-900 md:mb-4 md:text-5xl lg:mb-6 lg:max-w-3xl lg:text-6xl tracking-tight uppercase">
            {heading}
          </h2>
          <p className="mb-6 md:mb-8 text-slate-600 font-bold text-sm md:text-lg lg:max-w-2xl lg:text-xl">
            {description}
          </p>
          ...
          {posts.map((post) => (
            <Card key={post.id} className="flex flex-col border-none bg-sky-50/30 rounded-xl md:rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 group h-full">
              ...
              <CardHeader className="p-6 md:p-8 flex-1">
                <h3 className="text-lg md:text-2xl font-black font-headline text-slate-900 leading-tight group-hover:text-primary transition-colors uppercase">
                  <a href={post.url}>
                    {post.title}
                  </a>
                </h3>
